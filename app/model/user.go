...
import (
	...
	"go-admin/app/service/users/token_cache_redis"
	"go-admin/app/utils/md5_encrypt"
	"go.uber.org/zap"
	"time"
)
...
func (u *UsersModel) UpdateUserloginInfo(lastLoginIp string, userId int64) {
	...
	_ = u.Exec(sql, lastLoginIp, time.Now().Format(variable.DateFormat), userId)
}
...
// 删除用户以及关联的token记录
func (u *UsersModel) Destory(id int) bool {
	// 删除用户时，清除用户缓存在redis1的全部token
	if variable.ConfigYml.GetInt("Token.IsCacheToRedis") == 1 {
